...
public class SqliteCampaignRepositoryTests : IAsyncLifetime
{
    ...
    {
        var campaign = Campaign.Create("Ashes", DateTimeOffset.UtcNow); campaign.SelectRuleset(new RulesetReference("example-fantasy", "1.0.0"), DateTimeOffset.UtcNow); await repository.SaveAsync(campaign); await repository.SetSelectedIdAsync(campaign.Id);
