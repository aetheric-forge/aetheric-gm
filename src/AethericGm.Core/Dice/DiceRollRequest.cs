...
public sealed record DiceRollRequest
{
    ...
    public DiceRollRequest(int count, int sides, int modifier = 0, DiceRollMode mode = DiceRollMode.Normal, string? label = null, DiceRollContext? context = null)
    {
        ...
        Label = string.IsNullOrWhiteSpace(label) ? "Untitled roll" : label.Trim(); Context = context;
    }
