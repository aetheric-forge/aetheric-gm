...
public sealed class SqliteCampaignRepository(string connectionString) : ICampaignRepository
{
    ...
    public async Task<IReadOnlyList<Campaign>> ListAsync(bool includeArchived = false, CancellationToken ct = default)
    {
        await using var connection = await OpenAsync(ct);
        var command = connection.CreateCommand();
        command.CommandText = $"SELECT id,name,system,setting,summary,created_at,updated_at,archived_at FROM campaigns {(includeArchived ? "" : "WHERE archived_at IS NULL")} ORDER BY updated_at DESC";
    ...
    public async Task<Campaign?> GetAsync(Guid id, CancellationToken ct = default)
    {
        await using var connection = await OpenAsync(ct);
        var command = connection.CreateCommand();
        command.CommandText = "SELECT id,name,system,setting,summary,created_at,updated_at,archived_at FROM campaigns WHERE id=$id";
    ...
    public async Task SaveAsync(Campaign campaign, CancellationToken ct = default)
    {
        ...
        var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO campaigns(id,name,system,setting,summary,created_at,updated_at,archived_at)
            VALUES($id,$name,$system,$setting,$summary,$created,$updated,$archived)
            ON CONFLICT(id) DO UPDATE SET name=$name,system=$system,setting=$setting,summary=$summary,updated_at=$updated,archived_at=$archived;
            """;
